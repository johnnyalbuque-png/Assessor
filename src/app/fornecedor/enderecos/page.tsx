...
export default function EnderecosPage() {
  ...
  async function carregar() {
    const res = await fetch("/api/fornecedor/enderecos");
  ...
  async function salvar(e: React.FormEvent) {
    ...
    try {
      const url = editId ? `/api/fornecedor/enderecos/${editId}` : "/api/fornecedor/enderecos";
  ...
  async function excluir(id: string) {
    ...
    await fetch(`/api/fornecedor/enderecos/${id}`, { method: "DELETE" });
    await carregar();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Endereços / Lojas</h1>
        {!editId && (
