...
const EMPTY = { nome: "", descricao: "", preco: "", imagem: "", ordem: 0 };
...
export default function ProdutosPage() {
  ...
  function iniciarEdicao(p: Produto) {
    setEditId(p.id);
    setForm({ nome: p.nome, descricao: p.descricao ?? "", preco: p.preco ?? "", imagem: p.imagem ?? "", ordem: p.ordem });
    setErro("");
  }
  ...
  async function salvar(e: React.FormEvent) {
    ...
    try {
      ...
      const payload = { ...form, imagem: form.imagem || null, preco: form.preco || null, descricao: form.descricao || null };
  ...
  async function toggleAtivo(p: Produto) {
    await fetch(`/api/fornecedor/produtos/${p.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ativo: !p.ativo }) });
    await carregar();
  }
        ...
        <form onSubmit={salvar} className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input className={inp} required value={form.nome} onChange={(e) => setForm(f => ({ ...f, nome: e.target.value }))} placeholder="Ex: Suporte técnico 24h" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Preço</label>
              <PriceInput value={form.preco} onChange={(v) => setForm(f => ({ ...f, preco: v }))} placeholder="299,90" />
              <p className="text-xs text-gray-400 mt-1">Digite o valor ou texto (ex: Consultar, Sob consulta)</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Ordem (menor = primeiro)</label>
              <input type="number" className={inp} value={form.ordem} onChange={(e) => setForm(f => ({ ...f, ordem: Number(e.target.value) }))} />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Descrição</label>
              <textarea className={`${inp} min-h-[80px] resize-y`} value={form.descricao} onChange={(e) => setForm(f => ({ ...f, descricao: e.target.value }))} placeholder="Breve descrição..." />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Imagem (opcional)</label>
              <ImageUpload value={form.imagem} onChange={(url) => setForm(f => ({ ...f, imagem: url }))} tipo="default" aspect="wide" />
            </div>
          </div>
          ...
          {produtos.map((p) => (
            <div key={p.id} className={`bg-white rounded-xl border p-4 flex items-start gap-4 ${p.ativo ? "border-gray-100" : "border-gray-100 opacity-60"}`}>
              {p.imagem && (
                ...
                <img src={p.imagem} alt={p.nome} className="w-16 h-16 rounded-lg object-cover shrink-0 border border-gray-100" />
              )}
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-gray-800">{p.nome}</h3>
                  {p.preco && <span className="text-xs font-bold text-[#1B3A6B] bg-blue-50 px-2 py-0.5 rounded-full">R$ {p.preco}</span>}
