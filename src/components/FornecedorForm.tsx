...
export default function FornecedorForm({ categorias, inicial }: { categorias: Categoria[]; inicial?: FornecedorData }) {
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 p-8 space-y-6 max-w-2xl">
      <div className="grid sm:grid-cols-2 gap-5">
        <div>
          ...
          <input name="nome" defaultValue={inicial?.nome} required className={field} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Slug (URL) *</label>
          <input name="slug" defaultValue={inicial?.slug} required placeholder="ex: intelbras-telecom" className={field} />
        </div>
      </div>

      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Descrição *</label>
        <textarea name="descricao" defaultValue={inicial?.descricao} required rows={4} className={`${field} resize-none`} />
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {REGIOES.map((r) => (
            <button key={r} type="button" onClick={() => toggleRegiao(r)}
              className={`px-4 py-1.5 rounded-full text-sm border transition-colors ${regioes.includes(r) ? "bg-[#1B3A6B] text-white border-[#1B3A6B]" : "border-gray-200 text-gray-600 hover:border-[#2E86AB]"}`}>
              {r}
            </button>
          ))}
        </div>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <div>
          ...
          <input name="whatsapp" defaultValue={inicial?.whatsapp ?? ""} placeholder="11999990000" className={field} />
        </div>
        <div>
        ...
        <div>
          ...
          <input name="video" defaultValue={inicial?.video ?? ""} placeholder="https://youtube.com/embed/..." className={field} />
        </div>
      </div>

      <div className="flex gap-6">
        <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
          <input name="ativo" type="checkbox" defaultChecked={inicial?.ativo ?? true} className="w-4 h-4 accent-[#1B3A6B]" />
          Perfil ativo
        </label>
        <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
          <input name="recomendado" type="checkbox" defaultChecked={inicial?.recomendado ?? false} className="w-4 h-4 accent-[#2E86AB]" />
